...
const SwipeFeed = () => {
    ...
    const handlePointerUp = (e, product) => {
        ...
        // CHECK THRESHOLD FOR ANIMATION
        if (dragOffset.x > 120 || dragOffset.x < -120) {
            setIsFlyingAway(true);
            const direction = dragOffset.x > 120 ? "right" : "left";

            // Wait 300ms for the animation to finish, then cycle the data
            setTimeout(() => {
                handleSwipeAction(direction, product);
            }, 300);
        } else {
            // If they didn't swipe far enough, snap back to center
            setDragOffset({ x: 0, y: 0 });
        }
    };
    ...
    const handleSwipeAction = (direction, product) => {
            ...
            if (category !== "item" && category !== "kit") {
                setLeftSwipeMemory((prev) => {
                    ...
                    if (count === 2 && !bannedTerms.includes(category)) {
                        ...
                        setToastMessage(
                            `Got it. Showing fewer "${category}" suggestions.`,
                        );
        ...
        setProducts((prev) =>
            prev.filter((p) => p.id !== (product.id || product.tier_name)),
        );
        setDragOffset({ x: 0, y: 0 });
    ...
    const cartTotal = cartItems.reduce(
        (sum, item) =>
            sum + (item.type === "bundle" ? item.total_price : item.price),
        0,
    );
                    ...
                    {cartItems.length === 0 ? (
                        ...
                        <div className="flex flex-col gap-4 pb-8">
                            {cartItems.map((item, i) => (
                                <div
                                    key={i}
                                    className="bg-white p-4 rounded-2xl shadow-sm border flex items-center gap-4 shrink-0"
                                >
                                    {item.type === "bundle" ? (
                                        <div className="w-16 h-16 rounded-xl bg-purple-50 flex items-center justify-center text-2xl border border-purple-100">
                                            🎁
                                        </div>
                                    ) : (
                                        <img
                                            src={`/api/images/product_${item.id}.jpg`}
                ...
                {cartItems.length > 0 && (
                    <div className="bg-white px-6 py-6 border-t border-gray-200 shrink-0 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] z-20 pb-safe">
                        ...
                        <button
                            onClick={() => {
                                alert(
                                    `Redirecting to Secure Payment Gateway to pay ₹${cartTotal}... \n\n(Demo Checkout Success!)`,
                                );
                                setCartItems([]);
                                setCurrentView("feed");
                            }}
    ...
    const visibleProducts = products.filter((p) => {
        if (p.type === "bundle") {
            ...
            return !p.items?.some(
                (subItem) =>
                    subItem.category &&
                    bannedTerms.includes(subItem.category.toLowerCase()),
            );
        }
    ...
    const cardStyle = isDragging
        ...
        : {
              transform: isFlyingAway
                  ? `translate3d(${dragOffset.x > 0 ? 1000 : -1000}px, ${dragOffset.y}px, 0) rotate(${dragOffset.x * 0.1}deg)`
    ...
    return (
        <div className="relative w-full h-full p-4 flex flex-col pt-8 bg-gray-50 overflow-hidden">
            <div
                ...
                className="absolute top-6 right-6 z-50 bg-white p-3 rounded-full shadow-md cursor-pointer flex items-center gap-2 transition-transform hover:scale-105 active:scale-95"
            >
                <span>🛒</span>{" "}
                <span className="font-black">{cartItems.length}</span>
            </div>

            <h1 className="text-3xl font-black text-center mb-4 tracking-tighter mt-4">
                NUDGE
            </h1>

            <form
                onSubmit={handleSearchSubmit}
                className="flex gap-2 mb-4 px-2 z-20"
            >
                <input
                    value={inputValue}
                    onChange={(e) => setInputValue(e.target.value)}
                    placeholder="What's on your mind?"
                    className="flex-1 rounded-full px-5 py-3 shadow-sm border border-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
                <button
                    type="submit"
                    className="bg-gray-900 text-white rounded-full px-5 py-3 font-bold shadow-md active:scale-95 transition-transform"
                >
                    Go
                </button>
            </form>
            ...
            <div
                className={`absolute top-24 left-0 right-0 mx-auto w-max z-50 transition-all duration-500 transform ${toastMessage ? "translate-y-0 opacity-100" : "-translate-y-4 opacity-0 pointer-events-none"}`}
            >
            ...
            <div
                className={`w-full max-w-sm mx-auto absolute top-40 left-0 right-0 z-20 px-6 transition-all duration-700 ease-in-out ${showAiChip && aiContext && !aiContext.is_feed && !loading ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-4 pointer-events-none"}`}
            >
                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100 rounded-2xl p-3 shadow-md">
                    <div className="flex items-start gap-3">
                        <div className="bg-white p-2 rounded-full shadow-sm text-lg">
                            ✨
                        </div>
                        <div className="flex-1 pt-0.5">
                            <p className="text-xs font-black text-blue-600 uppercase tracking-wider mb-0.5">
                                AI Understood
                            </p>
                            ...
                            {aiContext && aiContext.dropped_count > 0 && (
                                <div className="mt-2 flex items-center gap-1.5 text-xs font-bold text-slate-500 bg-white/60 w-fit px-2 py-1 rounded-md border border-slate-200/60">
                                    ...
                                    Strictly removed {aiContext.dropped_count}{" "}
                                    items exceeding limits
                                </div>
                            )}
                            {/* --------------------------------------- */}
                        </div>
            ...
            <div className="relative flex-1 w-full flex justify-center items-center mt-2">
                {loading && products.length === 0 ? (
                    ...
                    <>
                            ...
                            <div className="absolute w-80 h-[28rem] bg-white rounded-3xl border border-gray-200 scale-95 translate-y-5 opacity-70 z-0 flex flex-col overflow-hidden shadow-sm">
                                <div className="h-3/5 relative border-b border-gray-100 bg-gray-100 flex items-center justify-center">
                                    {nextProduct.type === "bundle" ? (
                                        ...
                                        <img
                                            src={`/api/images/product_${nextProduct.id}.jpg`}
                                            className="w-full h-full object-cover grayscale-[30%] blur-[1px] pointer-events-none"
                                            onError={(e) =>
                                                (e.target.src =
                        ...
                        <div
                            onPointerDown={(e) =>
                                handlePointerDown(e, activeProduct)
                            }
                            onPointerMove={handlePointerMove}
                            onPointerUp={(e) =>
                                handlePointerUp(e, activeProduct)
                            }
                            style={cardStyle}
                            ...
                            {activeProduct.type === "bundle" ? (
                                ...
                                <>
                                    ...
                                    {showBreakdown && (
                                        <div className="absolute inset-0 z-50 bg-gray-900/95 backdrop-blur-md p-6 flex flex-col justify-center text-white transition-opacity duration-200">
                                            ...
                                            <div className="space-y-5 overflow-y-auto max-h-[80%] pb-4">
                                                {activeProduct.items?.map(
                                                    (subItem, idx) => (
                                                        <div
                                                            key={idx}
                                                            className="border-b border-gray-700/50 pb-3"
                                                        >
                                                            ...
                                                            <p className="text-sm text-gray-400 italic flex items-start gap-2">
                                                                ...
                                                                {subItem.user_context ||
                                                                    "Semantic match for current search intent."}
                                                            </p>
                                                        </div>
                                                    ),
                                                )}
                                    ...
                                    <div className="h-3/5 relative border-b border-purple-100 bg-gradient-to-br from-purple-50 to-indigo-50 flex items-center justify-center p-4">
                                        <div className="grid grid-cols-2 gap-2 w-full max-w-[220px] pointer-events-none">
                                            {activeProduct.items
                                                ...
                                                .map((subItem, idx) => (
                                                    <div
                                                        key={idx}
                                                        className="relative w-full pt-[100%] rounded-xl overflow-hidden shadow-sm border border-white/60"
                                                    >
                                                        <img
                                                            src={`/api/images/product_${subItem.id}.jpg`}
                                        ...
                                        {dragOffset.x > 40 && (
                                            <div className="absolute top-6 left-6 bg-green-500 text-white text-sm font-black px-4 py-1.5 rounded-lg rotate-[-12deg] uppercase shadow-lg z-10">
                                                Add Bundle
                                            </div>
                                        )}
                                        {dragOffset.x < -40 && (
                                            <div className="absolute top-6 right-6 bg-red-500 text-white text-sm font-black px-4 py-1.5 rounded-lg rotate-[12deg] uppercase shadow-lg z-10">
                                                Skip
                                            </div>
                                        )}
                                    </div>
                                ...
                                <>
                                    <div className="h-3/5 relative border-b border-gray-100 bg-gray-100">
                                        ...
                                        />
                                        {dragOffset.x > 40 && (
                                            <div className="absolute top-6 left-6 bg-green-500 text-white text-sm font-black px-4 py-1.5 rounded-lg rotate-[-12deg] uppercase shadow-lg">
                                                Add to Cart
                                            </div>
                                        )}
                                        {dragOffset.x < -40 && (
                                            <div className="absolute top-6 right-6 bg-red-500 text-white text-sm font-black px-4 py-1.5 rounded-lg rotate-[12deg] uppercase shadow-lg">
                                                Skip
                                            </div>
                                        )}
                                    </div>
