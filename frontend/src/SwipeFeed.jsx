              ...
              {cartItems.map((item, i) => (
                <div key={i} className="bg-white p-4 rounded-2xl shadow-sm border flex items-center gap-4 shrink-0">
                  {item.type === 'bundle' ? (
                    ...
                    <img draggable="false" src={`/api/images/product_${item.id}.jpg`} className="w-16 h-16 rounded-xl object-cover select-none" onError={(e) => e.target.src = 'https://via.placeholder.com/150'} />
                  )}
      ...
      <div className="relative flex-1 w-full flex justify-center items-center mt-2">
        {loading && visibleProducts.length === 0 ? (
          ...
          <>
            ...
            {nextProduct && (
              <div className="absolute w-80 h-[28rem] bg-white rounded-3xl border border-gray-200 scale-95 translate-y-5 opacity-70 z-0 flex flex-col overflow-hidden shadow-sm select-none">
                <div className="h-3/5 relative border-b border-gray-100 bg-gray-100 flex items-center justify-center">
                  {nextProduct.type === 'bundle' ? (
                    ...
                    <img draggable="false" src={`/api/images/product_${nextProduct.id}.jpg`} className="w-full h-full object-cover grayscale-[30%] blur-[1px] pointer-events-none select-none" onError={(e) => e.target.src = 'https://via.placeholder.com/300?text=No+Image'} />
                  )}
                </div>
            ...
            <div 
              ...
              style={cardStyle} 
              className="absolute w-80 h-[28rem] bg-white rounded-3xl shadow-2xl overflow-hidden cursor-grab active:cursor-grabbing z-10 border border-gray-100 flex flex-col transition-shadow hover:shadow-blue-900/10 touch-none select-none"
            >
              ...
              {activeProduct.type === 'bundle' ? (
                    ...
                    <div className="grid grid-cols-2 gap-2 w-full max-w-[220px] pointer-events-none">
                      {activeProduct.items?.slice(0, 4).map((subItem, idx) => (
                        <div key={idx} className="relative w-full pt-[100%] rounded-xl overflow-hidden shadow-sm border border-white/60">
                          <img draggable="false" src={`/api/images/product_${subItem.id}.jpg`} className="absolute inset-0 w-full h-full object-cover select-none" onError={(e) => e.target.src = 'https://via.placeholder.com/150'} />
                        </div>
                      ))}
                    </div>
                ...
                <>
                  <div className="h-3/5 relative border-b border-gray-100 bg-gray-100">
                    <img draggable="false" src={`/api/images/product_${activeProduct.id}.jpg`} className="w-full h-full object-cover pointer-events-none select-none" onError={(e) => e.target.src = 'https://via.placeholder.com/300?text=No+Image'} />
